...
namespace DefaultNamespace
{
    public class HttpTester : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            http.GetAsync<Player>("player/2").ContinueWith(EnqueueShowPlayer);
        ...
        private void Start()
        {
            http.PostAsync<string, string>("auth/signout", "").ContinueWith(EnqueueShowGeneric);
        }
        ...
        private void Post()
        {
            ...
            http.PostAsync<LoginModelView, LoginModelView>("auth/signin", new LoginModelView((idField.text),  nameField.text)).ContinueWith(EnqueueShowGeneric);
